...
# Initialize model
model = MusicTaggerCRNN(weights=None, input_tensor=(96, 1366, 1))
...
if LOAD_WEIGHTS:
    #model.load_weights(weights_path+'crnn_net_gru_adam_ours_epoch_40.h5', reshape=True)
    model.load_weights(weights_path + 'new1.h5')
#model.summary()
...
#model.save_weights(weights_path+'my_weights_tensorflow_1.h5')
...
for i in range(0, num_frames_test.shape[0]):
    ...
    for j in range(previous_numFrames, previous_numFrames+num_frames):
        ...
        print('Percentage of genre prediction for seconds '+ str(20+s_counter*30) + ' to ' + str(20+(s_counter+1)*30) + ': ')
